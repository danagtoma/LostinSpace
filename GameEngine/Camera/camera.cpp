...
void Camera::outOfBounds(float raza)
{
	if (pow(cameraPosition.x, 2) + pow(cameraPosition.z, 2) > pow(raza, 2)) {
		if (cameraPosition.x > 0) cameraPosition.x -= 0.1f;
		else cameraPosition.x += 0.1f;
		if (cameraPosition.z > 0) cameraPosition.z -= 0.1f;
		else cameraPosition.z += 0.1f;
	}
}
